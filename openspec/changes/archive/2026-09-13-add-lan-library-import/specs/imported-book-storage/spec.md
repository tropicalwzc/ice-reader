...
The app SHALL persist imported-book metadata independently from the hard-coded bundled list, including a stable local ID, title, extension, relative file path, source server ID, source book ID, content hash, size, and import date.
...
The app SHALL bundle only the readable `样例占位.txt` novel and SHALL obtain all other novels through the imported-book catalog without deleting the sibling server-library source files.
...
The app SHALL store imported content beneath its Application Support directory, download into a temporary file, verify advertised byte count and SHA-256, and atomically install or replace the final file before committing catalog changes.
...
#### Scenario: Remote title changes
- **WHEN** an imported server book is renamed and later updated under the same server book ID
- **THEN** the app updates its display metadata without losing its local reading progress
...
#### Scenario: Same-title book is downloaded from another source
- **WHEN** a verified download has the same normalized title as an existing imported book but a different server or server-book ID
