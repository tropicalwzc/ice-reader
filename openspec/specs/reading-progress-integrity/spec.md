# reading-progress-integrity Specification

## Purpose
Define how the reader determines, persists, and restores the user's reading position, and how that position relates to the scroll viewport, so that background work, lifecycle transitions, layout changes, and cloud alignment never move the reader's position without an explicit user action.
